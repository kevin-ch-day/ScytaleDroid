#!/usr/bin/env python3
"""Phase B1 join-key collation/width normalization preflight and apply entrypoint."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    report.add_argument(
        "--receipt-dir",
        type=Path,
        default=_REPO_ROOT / "data" / "state" / "schema_migrations" / "phase_b1_join_key_normalization",
    ...
    apply.add_argument(
        "--apply",
        action="store_true",
        help="Run the live B1 ALTER/view-recreate migration.",
    )
...
def _handle_report(args: argparse.Namespace) -> int:
    (
        core_q,
        _apply_phase_b1_join_key_normalization,
        _build_required_alter_statements,
        collect_phase_b1_join_key_preflight,
        write_phase_b1_join_key_preflight_bundle,
    ) = _load_runtime_dependencies()

    report = collect_phase_b1_join_key_preflight(
        core_q.run_sql,
        sample_limit=max(1, min(int(args.sample_limit), 100)),
    )
    stamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
    stem = f"phase_b1_join_key_normalization_{stamp}"
    files = write_phase_b1_join_key_preflight_bundle(report, args.receipt_dir, stem=stem)
    payload = {
    ...
    print("# phase-b1 join-key normalization preflight")
...
def _handle_apply(args: argparse.Namespace) -> int:
    (
        ...
        build_required_alter_statements,
        collect_phase_b1_join_key_preflight,
        _write_phase_b1_join_key_preflight_bundle,
    ) = _load_runtime_dependencies()
