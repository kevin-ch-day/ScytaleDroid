...
def build_behavior_report(
    ...
    if metadata.get("telemetry_status") and metadata.get("telemetry_status") != "ok":
        lines.append(f"- Telemetry degraded: {metadata.get('telemetry_status')}.")
    ...
    lines.append(f"- Telemetry available: {metadata.get('telemetry_available')} ({metadata.get('telemetry_status')})")
    lines.append(f"- UID resolution: {metadata.get('uid_resolution')}")
    lines.append(f"- PID resolution: {metadata.get('pid_resolution')}")
    ...
    if metadata.get("network_status"):
        lines.append(f"- Network status: {metadata.get('network_status')}")
    ...
    lines.append(f"- Network source summary: {ns}")
    lines.append(f"- Telemetry status: {metadata.get('telemetry_status')}")
    lines.append(f"- Network status: {metadata.get('network_status')}")
    lines.append(f"- Missed samples: {metadata.get('missed_sample_count')}")
    lines.append("")
