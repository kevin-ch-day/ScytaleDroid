...
def about_app() -> None:
    ...
    if app_config.SHOW_RUNTIME_IDENTITY:
        print()
        menu_utils.print_section("Runtime Mode")
        metrics = [
            ("Runtime preset", app_config.RUNTIME_PRESET.upper()),
            ("Execution mode", app_config.EXECUTION_MODE),
            ("System environment", app_config.SYS_ENV),
            ("Debug mode", "ON" if app_config.DEBUG_MODE else "OFF"),
        ]
        if app_config.SYS_TEST:
            metrics.append(("System test", "ON"))
