...
def _latest_governance_status() -> tuple[str, str | None, str | None, int, str | None]:
    try:
        version, sha, rows = intel_db.latest_governance_snapshot()
        if version:
            loaded_at_value = intel_db.latest_governance_loaded_at(version)
            return "present", str(version), str(sha or ""), int(rows or 0), str(loaded_at_value or "")
