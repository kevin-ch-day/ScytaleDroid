...
def _intel_table_exists(table: str) -> bool:
    try:
        return intel_db.intel_table_exists(table)
    except Exception:
        return False
...
def ensure_signal_catalog() -> bool:
    ...
    ok = _intel_table_exists(intel_db.SIGNAL_CATALOG_TABLE)
    if not ok:
        log.warning(
            f"{intel_db.SIGNAL_CATALOG_TABLE} missing; run DBA migrations.",
            category="database",
        )
    return ok
...
def ensure_signal_mappings() -> bool:
    ...
    ok = _intel_table_exists(intel_db.SIGNAL_MAPPINGS_TABLE)
    if not ok:
        log.warning(
            f"{intel_db.SIGNAL_MAPPINGS_TABLE} missing; run DBA migrations.",
            category="database",
        )
    return ok
...
def ensure_cohort_expectations() -> bool:
    ...
    ok = _intel_table_exists(intel_db.COHORT_EXPECTATIONS_TABLE)
    if not ok:
        log.warning(
            f"{intel_db.COHORT_EXPECTATIONS_TABLE} missing; run DBA migrations.",
            category="database",
        )
    return ok
...
def ensure_all() -> dict[str, bool]:
    ...
    operations: tuple[tuple[str, Callable[[], bool]], ...] = (
        (intel_db.AOSP_DICT_TABLE, lambda: _intel_table_exists(intel_db.AOSP_DICT_TABLE)),
        (intel_db.OEM_DICT_TABLE, lambda: _intel_table_exists(intel_db.OEM_DICT_TABLE)),
        (intel_db.UNKNOWN_DICT_TABLE, lambda: _intel_table_exists(intel_db.UNKNOWN_DICT_TABLE)),
        (intel_db.QUEUE_DICT_TABLE, lambda: _intel_table_exists(intel_db.QUEUE_DICT_TABLE)),
        (intel_db.OEM_VENDOR_META_TABLE, lambda: _intel_table_exists(intel_db.OEM_VENDOR_META_TABLE)),
        (intel_db.OEM_PREFIX_META_TABLE, lambda: _intel_table_exists(intel_db.OEM_PREFIX_META_TABLE)),
        (intel_db.SIGNAL_CATALOG_TABLE, ensure_signal_catalog),
        (intel_db.SIGNAL_MAPPINGS_TABLE, ensure_signal_mappings),
        (intel_db.COHORT_EXPECTATIONS_TABLE, ensure_cohort_expectations),
...
def seed_signal_catalog(
    ...
    try:
        existing_rows = intel_db.fetch_signal_catalog_rows()
    except Exception:
        existing_rows = []
    ...
    for raw_payload in payloads:
        ...
        if current is None:
            try:
                intel_db.insert_signal_catalog_row(payload)
                inserted += 1
                existing[key] = {
                    "signal_key": key,
        ...
        try:
            intel_db.update_signal_catalog_row(payload)
            updated += 1
            existing[key] = {
                "signal_key": key,
                "display_name": payload["display_name"],
...
def seed_defaults() -> dict[str, dict[str, int]]:
    ...
    return {intel_db.SIGNAL_CATALOG_TABLE: seed_signal_catalog()}
