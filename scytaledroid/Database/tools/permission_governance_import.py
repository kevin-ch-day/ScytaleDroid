...
def _insert_entries(snapshot_id: str, rows: Iterable[Mapping[str, object]], *, db) -> int:
    count = 0
    for row in rows:
        payload = {
        ...
        db.execute(
            """
            INSERT INTO """
            + intel_db.GOVERNANCE_ROWS_TABLE
            + """ (
              governance_version,
              permission_string,
              namespace_type,
            ...
            """,
            payload,
            query_name="permission_intel.import.upsert_row",
        )
        count += 1
    return count
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        with intel_db.session() as db:
            with db.transaction():
                db.execute(
                    """
                    INSERT INTO """
                    + intel_db.GOVERNANCE_SNAPSHOTS_TABLE
                    + """ (governance_version, snapshot_sha256, source_system)
                    VALUES (%s, %s, %s)
                    ON DUPLICATE KEY UPDATE snapshot_sha256 = VALUES(snapshot_sha256), source_system = VALUES(source_system)
                    """,
                    (version, snapshot_hash, args.source or "EREBUS"),
                    query_name="permission_intel.import.upsert_snapshot",
                )
                snapshot_id = version
                inserted = _insert_entries(snapshot_id, rows, db=db)
    except Exception as exc:
