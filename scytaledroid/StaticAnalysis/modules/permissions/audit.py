...
def _extract_static_run_id_map(payload: Any) -> dict[str, int]:
...
def _build_snapshot_lineage(
    *,
    ...
    run_map_payload: Any,
    static_run_id_map: Mapping[str, int],
    run_map_required: bool,
) -> dict[str, Any]:
    normalized_map = {str(pkg): int(srid) for pkg, srid in static_run_id_map.items()}
    sorted_static_run_ids = sorted(set(normalized_map.values()))
    run_map_digest = hashlib.sha256(
    ...
    lineage: dict[str, Any] = {
        ...
        "run_map_required": bool(run_map_required),
        "run_map_present": bool(run_map_payload),
        "run_map_sha256": run_map_digest,
        "run_map_packages": len(normalized_map),
        "static_run_ids": sorted_static_run_ids,
        "static_run_count": len(sorted_static_run_ids),
        "static_run_min": sorted_static_run_ids[0] if sorted_static_run_ids else None,
        "static_run_max": sorted_static_run_ids[-1] if sorted_static_run_ids else None,
    }
    if static_run_id is None and len(sorted_static_run_ids) == 1:
        lineage["static_run_id"] = sorted_static_run_ids[0]
    return lineage
...
class PermissionAuditAccumulator:
    ...
    def add_app(
        ...
        *,
        ...
        raw_score = float(
            score_detail.get(
                "score_raw",
                score_detail.get("score_3dp", score_detail.get("score_capped", 0.0)),
            )
            or 0.0
        )
        capped_score = float(
            score_detail.get(
                "score_capped",
                score_detail.get("score_3dp", raw_score),
            )
            or raw_score
        )
        capped_score = max(0.0, min(10.0, capped_score))
        score_detail["score_raw"] = raw_score
        score_detail["score_capped"] = capped_score
        score_detail["score_3dp"] = round(capped_score, 3)
    ...
    def persist_to_db(self, snapshot_payload: dict[str, Any]) -> OperationResult:
        ...
        try:
            ...
            snapshot_payload = dict(snapshot_payload or {}) if isinstance(snapshot_payload, dict) else {}
            ...
            static_run_id_map: dict[str, int] = _extract_static_run_id_map(run_map_payload)
            normalized_package_map: dict[str, str] = {}
            for app in self.apps:
                raw_package = str(app.package or "").strip()
                if not raw_package:
                    continue
                identity = resolve_package_identity(raw_package, context="static_analysis")
                normalized_package = identity.normalized_package_name or raw_package
                normalized_package_map[raw_package] = normalized_package
            missing_run_ids = [
                app.package
                for app in self.apps
                if normalized_package_map.get(app.package, app.package) not in static_run_id_map
            ]
            ...
            lineage = _build_snapshot_lineage(
                ...
                run_id=run_id,
                static_run_id=static_run_id,
                run_map_payload=run_map_payload,
                static_run_id_map=static_run_id_map,
                run_map_required=run_map_required,
            )
            if static_run_id is None:
                stable_static_run_id = lineage.get("static_run_id")
                if isinstance(stable_static_run_id, int):
                    static_run_id = stable_static_run_id
            snapshot_payload["lineage"] = lineage
            ...
            def _persist_snapshot() -> OperationResult:
                ...
                for app in self.apps:
                    raw_package = str(app.package or "").strip()
                    normalized_package = normalized_package_map.get(raw_package, raw_package)
                    app_static_run_id = run_id_map.get(normalized_package, run_id_map.get(raw_package, static_run_id_local))
