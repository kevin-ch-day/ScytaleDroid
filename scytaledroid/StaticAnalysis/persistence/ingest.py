    ...
    for result in results:
        ...
        for entry in findings:
            ...
            context[finding_id] = {
                ...
                "masvs_area": entry.get("masvs_area") or metrics.get("masvs_area"),
                "masvs_control_id": entry.get("masvs_control_id") or metrics.get("masvs_control_id"),
...
def _get_or_create_version(
    ...
    *,
    ...
    def _is_placeholder_version_name(value: object) -> bool:
        if value is None:
            return True
        try:
    ...
    try:
        row = core_q.run_sql(
            "SELECT id, min_sdk, target_sdk FROM app_versions WHERE app_id = %s AND version_name <=> %s AND version_code <=> %s",
            (app_id, version_name, version_code),
            fetch="one",
        )
        if row and row[0]:
            version_id = int(row[0])
            existing_min_sdk = row[1] if len(row) > 1 else None
            existing_target_sdk = row[2] if len(row) > 2 else None
            if ((existing_min_sdk is None and min_sdk is not None) or (existing_target_sdk is None and target_sdk is not None)):
                core_q.run_sql(
                    """
                    UPDATE app_versions
                    SET min_sdk = COALESCE(min_sdk, %s),
                        target_sdk = COALESCE(target_sdk, %s)
                    WHERE id = %s
                    """,
                    (min_sdk, target_sdk, version_id),
                )
            return version_id
        if version_code is not None:
            row = core_q.run_sql(
                """
                SELECT id, version_name, min_sdk, target_sdk
                FROM app_versions
                WHERE app_id = %s AND version_code = %s
                ORDER BY
                  CASE
                    WHEN version_name IS NULL OR TRIM(version_name) IN ('', '-', '—') THEN 1
                    ELSE 0
                  END ASC,
                  CASE WHEN target_sdk IS NULL THEN 1 ELSE 0 END ASC,
                  CASE WHEN min_sdk IS NULL THEN 1 ELSE 0 END ASC,
                  id DESC
                LIMIT 1
                """,
                (app_id, version_code),
            ...
            if row and row[0]:
                version_id = int(row[0])
                existing_version_name = row[1] if len(row) > 1 else None
                existing_min_sdk = row[2] if len(row) > 2 else None
                existing_target_sdk = row[3] if len(row) > 3 else None
                if (
                    (_is_placeholder_version_name(existing_version_name) and not _is_placeholder_version_name(version_name))
                    or (existing_min_sdk is None and min_sdk is not None)
                    or (existing_target_sdk is None and target_sdk is not None)
                ):
                    core_q.run_sql(
                        """
                        UPDATE app_versions
                        ...
                        WHERE id = %s
                        """,
                        (version_name, min_sdk, target_sdk, version_id),
                    )
                return version_id
        new_id = core_q.run_sql(
            (
