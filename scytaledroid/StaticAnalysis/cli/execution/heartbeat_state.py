...
def set_run(session_stamp: str | None, *, phase: str = "scan") -> None:
    now = time.monotonic()
    with _LOCK:
        _STATE.session_stamp = str(session_stamp).strip() if session_stamp else None
        _STATE.phase = str(phase or "scan").strip() or "scan"
        _STATE.app_label = None
        _STATE.stage = None
        _STATE.done = None
        _STATE.total = None
...
def set_phase(phase: str, *, keep_app: bool = True) -> None:
    now = time.monotonic()
    with _LOCK:
        _STATE.phase = str(phase or "").strip() or _STATE.phase
        if not keep_app:
            _STATE.app_label = None
            _STATE.stage = None
            _STATE.done = None
...
def snapshot() -> dict[str, object]:
    with _LOCK:
        return {
            "session_stamp": _STATE.session_stamp,
            "phase": _STATE.phase,
...
__all__ = ["set_run", "set_phase", "set_app", "set_stage", "snapshot"]
