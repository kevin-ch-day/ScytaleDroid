...
def _run_map_entry_package(entry: Mapping[str, object]) -> str:
    """Return the best normalized package key for a run-map entry."""

    for key in ("normalized_package_name", "package", "manifest_package_name"):
        value = entry.get(key)
        if not value:
            continue
        identity = resolve_package_identity(str(value), context="static_analysis")
        if identity.normalized_package_name:
            return identity.normalized_package_name
    return ""
...
def validate_run_map(run_map: dict, session_stamp: str) -> None:
    ...
    for entry in apps:
        if not isinstance(entry, dict):
            continue
        package = _run_map_entry_package(entry) or entry.get("package")
...
def resolve_run_map_entry(
    ...
    for entry in apps:
        if not isinstance(entry, Mapping):
            continue
        entry_package = _run_map_entry_package(entry)
        app_package = resolve_package_identity(str(app_result.package_name or ""), context="static_analysis")
        if entry_package != app_package.normalized_package_name:
            continue
        static_run_id = entry.get("static_run_id")
