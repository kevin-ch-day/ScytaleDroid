...
def _baseline_severity_counts(
    finding_totals: Mapping[str, int],
    findings: Sequence[object] | None,
) -> Mapping[str, int]:
    if not isinstance(findings, Sequence):
        return coerce_severity_counts(finding_totals)

    derived = {"High": 0, "Medium": 0, "Low": 0, "Info": 0}
    for entry in findings:
        if not isinstance(entry, Mapping):
            continue
        severity = str(entry.get("severity") or "Info").strip().title()
...
def persist_static_sections(
    *,
    ...
    severity_counts = _baseline_severity_counts(finding_totals, findings_seq)
    details = {
