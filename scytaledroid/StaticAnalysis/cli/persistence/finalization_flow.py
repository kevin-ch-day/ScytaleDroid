...
def finalize_persisted_static_run(
    *,
    ...
    if not persistence_failed and enforced_session_label and paper_grade_requested:
        ...
        if is_group_scope:
            if enforced_session_label not in _GROUP_SCOPE_SKIP_LOGGED_SESSIONS:
                _GROUP_SCOPE_SKIP_LOGGED_SESSIONS.add(enforced_session_label)
                log.info(
                    "Skipping canonical singleton enforcement for group scope "
                    f"session_label={enforced_session_label}",
                    category="static_analysis",
                )
        else:
            try:
