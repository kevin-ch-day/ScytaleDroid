...
def _extract_declared_permissions(report: Any) -> Sequence[str]:
    try:
        perms = getattr(report, "permissions", None)
        declared = getattr(perms, "declared", None)
...
def compute_metrics_bundle(report: Any, string_data: Mapping[str, object]) -> MetricsBundle:
    declared = _extract_declared_permissions(report)
    flags = getattr(report, "manifest_flags", None)
    target_sdk = None
    try:
        target_sdk = _safe_int(getattr(getattr(report, "manifest", None), "target_sdk", None))
    except Exception:  # pragma: no cover - defensive
        target_sdk = None
...
def write_buckets(run_id: int, payload: Mapping[str, tuple[float, float]]) -> bool:
    return _dw.write_buckets(run_id, payload)


def write_metrics(run_id: int, payload: Mapping[str, tuple[float, float | None]]) -> bool:
    return _dw.write_metrics(run_id, payload)
