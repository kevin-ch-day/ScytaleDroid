...
def render_masvs_summary_menu() -> None:
    ...
    if run_info is None:
        print(status_messages.status("No canonical static runs available. Execute a full analysis first.", level="warn"))
        prompt_utils.press_enter_to_continue()
        return
    static_run_id, package_name, session_label = run_info
    ...
    print()
    subtitle = f"static_run_id={static_run_id}"
    if package_name:
        subtitle += f" | {package_name}"
    if session_label:
        subtitle += f" | {session_label}"
    menu_utils.print_header("MASVS Summary", subtitle)
    headers = [
        "Area",
        "High",
        "Med",
...
def _select_static_run() -> tuple[int, str | None, str | None] | None:
    try:
        rows = core_q.run_sql(
            """
            SELECT
    ...
    if not rows:
        return None
    options = [str(int(row.get("static_run_id") or 0)) for row in rows if row.get("static_run_id")]
    if not options:
        return None
    default = options[0]
    menu_utils.print_section("Available canonical static runs")
    for row in rows:
        static_run_id = row.get("static_run_id")
        if not static_run_id:
            continue
        app_label = row.get("app_label") or row.get("package_name") or "unknown"
        package_name = row.get("package_name") or "unknown"
        session_label = row.get("session_label") or "—"
        status = row.get("status") or "—"
        print(f"  - {static_run_id} | {app_label} | {package_name} | {session_label} | {status}")
    choice = prompt_utils.prompt_text("Enter static_run_id (default latest)", default=default, required=False)
    ...
    try:
        ...
        try:
            ...
            return None
    for row in rows:
        if int(row.get("static_run_id") or 0) == target:
            return (
                target,
                str(row.get("package_name")) if row.get("package_name") is not None else None,
                str(row.get("session_label")) if row.get("session_label") is not None else None,
            )
    try:
        return int(default), None, None
    except ValueError:
        return None
