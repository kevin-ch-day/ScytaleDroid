...
from .summary_format_helpers import preview_text, short_number
...
def _string_lines(string_payload: Mapping[str, object], *, verbose_output: bool) -> list[str]:
    ...
    for key, label in (
        ...
        if value:
            extra_pairs.append(f"{label}={short_number(value)}")
    if extra_pairs:
        lines.append("")
    ...
    if regex_skipped or noise_counts:
        noise_pairs = []
        if regex_skipped:
            noise_pairs.append(f"regex_skipped={short_number(regex_skipped)}")
        if isinstance(noise_counts, Mapping):
            for key, value in sorted(
                ...
                try:
                    ...
                    continue
                if count:
                    noise_pairs.append(f"{key}={short_number(count)}")
        if noise_pairs:
            lines.append("")
    ...
    for bucket in highlight_keys:
        ...
        for entry in shown_values:
            ...
            preview = preview_text(display_value or "(empty)")
    ...
    if clear_samples:
        ...
        for entry in top_clear:
            ...
            preview = preview_text(url, limit=70)
