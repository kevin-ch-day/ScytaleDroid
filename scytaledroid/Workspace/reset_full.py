...
Protected catalog tables (e.g., app/category catalogs, schema_version) are
...
def _reset_analysis_tables() -> ResetOutcome:
    """Truncate analysis result tables, preserving catalogs and harvest/library tables."""
...
def full_reset_files_and_db(*, project_root: Path) -> Path:
    ...
    # 1) DB reset (analysis tables only; preserve catalogs + harvest/library inputs).
