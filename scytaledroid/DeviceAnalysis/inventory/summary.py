...
def render_sync_summary_box(result) -> None:
    ...
    print(
        formatter.format_kv_block(
            ...
            {
                "Delta vs previous": (
                    f"new={new_count}  removed={removed_count}  "
                    f"{'metadata_changed' if updated_count > 0 and net_count_delta == 0 and not first_snapshot else 'updated'}={updated_count} "
    ...
    if snapshot_id is not None and expected_rows is not None and persisted_rows is not None:
        ...
        print(
            formatter.format_kv_block(
                "[DB]",
                {
                    "Rows": f"{persisted_rows}/{expected_rows}",
                    "Integrity": status,
                },
            )
        )
