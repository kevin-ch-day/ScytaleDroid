...
def extract_version_code_from_dump(package_dump: str, package_name: str) -> str | None:
    scoped = package_dump
    ...
    candidates: list[int] = []
    for pat in patterns:
        for match in re.finditer(pat, scoped):
            try:
    ...
    fallback = [int(match.group(1)) for match in re.finditer(r"versionCode=(\d+)\b", package_dump)]
