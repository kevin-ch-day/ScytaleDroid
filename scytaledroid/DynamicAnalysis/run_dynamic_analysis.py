...
def execute_dynamic_run_spec(spec: DynamicRunSpec) -> DynamicSessionResult:
    ...
    return run_dynamic_analysis(
        ...
        run_profile=getattr(spec, "run_profile", None),
        interaction_level=getattr(spec, "interaction_level", None),
        messaging_activity=getattr(spec, "messaging_activity", None),
        counts_toward_completion=getattr(spec, "counts_toward_completion", None),
