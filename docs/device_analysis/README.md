...
- Records the installer package, family hints (`com.android.*`, `com.google.*`,
  ...
  Messaging).
- Saves the snapshot on disk, writes retained rows to
  `device_inventory_snapshots` / `device_inventory`, and keeps the `apps` table
  synchronized with the latest package labels and profile metadata.
...
- `apps(id, package_name, display_name, profile_key, publisher_key, ...)`
  stores canonical app/package identity and is updated whenever new metadata is
  discovered during inventory sync.
...
- `harvest_source_paths` and `harvest_artifact_paths` preserve source-device and
  local-host lineage for each harvested `apk_id`.
...
## Inventory rerun and failure semantics

Current operator-visible behavior for physical-device inventory sync:

- writes happen at the end:
  - package metadata is collected in memory first
  - snapshot and DB sync happen only after collection finishes
  - if interrupted mid-collection, no new snapshot or DB changes are written
- delta criteria:
  - stable fields per package are compared:
    - `version_code`
    - `version_name`
    - `primary_path`
    - `split_count`
  - split-path reordering is normalized
  - benign reordering alone should not create a delta
- rerun behavior:
  - an immediate rerun after a successful sync reports identical-to-previous
    when those stable fields have not changed
  - runtime remains similar because collection is still fresh and uncached
- failure visibility:
  - a per-package failure during collection currently aborts the run
  - there is no partial snapshot written on failure today
- partial runs:
  - if the run stops before persistence, the next run resumes from the last
    complete snapshot
  - there is no separate incomplete marker yet

This is operator guidance for the current behavior, not a promise of future
guard semantics.

## Inventory guard determinism

Guard decisions must be deterministic for identical inputs.

Given the same:

- persisted inventory metadata state
- function inputs such as `serial` and scope package list
- guard configuration constants

the guard should return the same decision semantics.

Authoritative decision object fields:
...
Read/compute guard paths are expected to stay pure:
