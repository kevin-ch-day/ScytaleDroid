...
def test_governance_missing_blocks_publish(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(results.governance_ready.__globals__["intel_db"], "governance_snapshot_count", lambda: 0)
    monkeypatch.setattr(results.governance_ready.__globals__["intel_db"], "governance_row_count", lambda: 0)
...
def test_governance_present_allows_publish(monkeypatch, tmp_path, capsys):
    ...
    def _run_sql(query: str, *args, **kwargs):
        if "FROM artifact_registry" in query:
            return [(artifact,) for artifact in results.REQUIRED_PAPER_ARTIFACTS]
        return (0,)

    monkeypatch.setattr(results.core_q, "run_sql", _run_sql)
    monkeypatch.setattr(results.governance_ready.__globals__["intel_db"], "governance_snapshot_count", lambda: 1)
    monkeypatch.setattr(results.governance_ready.__globals__["intel_db"], "governance_row_count", lambda: 1)
