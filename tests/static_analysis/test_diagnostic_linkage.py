...
def test_linkage_valid_run_map_with_case_mismatch():
    run_map = {
        "apps": [
            {
                "package": "mnn.android",
                "manifest_package_name": "mnn.Android",
                "normalized_package_name": "mnn.android",
                "static_run_id": 7,
    ...
    status, note = diagnostics._diagnostic_linkage_status(
        _app("mnn.Android"), run_map=run_map, session_stamp="20260130-000000"
    )
    assert status == "VALID (run_map)"
    assert note == "static_run_id=7"
