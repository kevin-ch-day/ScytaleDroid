...
def test_render_results_fails_fast_when_run_id_missing(tmp_path: Path, monkeypatch) -> None:
    ...
    assert wrote["baseline"] is False
    assert "PERSISTENCE_ERROR" in outcome.failures


def test_render_results_fails_fast_on_persistence_error(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    now = datetime.now(UTC)
    manifest = SimpleNamespace(app_label="Example", package_name="com.example.app")
    report = SimpleNamespace(
    ...
    def _write_baseline_json(*_a, **_k):
        wrote["baseline"] = True
        out = tmp_path / "noncanonical.json"
...
def test_render_results_aborted_skips_post_run_views_and_db_masvs(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    now = datetime.now(UTC)
    manifest = SimpleNamespace(app_label="Tmp", package_name="tmp")
    report = SimpleNamespace(
    ...
    app = AppRunResult(package_name="tmp", category="Test", artifacts=[artifact], static_run_id=777)
    outcome = RunOutcome(
        results=[app],
        started_at=now,
        finished_at=now,
    ...
    called = {"post_views": 0, "cross_app": 0, "db_masvs": 0}
    monkeypatch.setattr(results, "analyse_strings", lambda *_a, **_k: {})
    monkeypatch.setattr(results, "_derive_highlight_stats", lambda *_a, **_k: {"providers": 0, "nsc_guard": 0, "secrets_suppressed": 0})
    monkeypatch.setattr(results, "_bulk_trend_deltas", lambda *_a, **_k: [])
    monkeypatch.setattr(results, "_apply_display_names", lambda *_a, **_k: None)
    ...
    monkeypatch.setattr(results, "write_dynamic_plan_json", lambda *_a, **_k: None)
    monkeypatch.setattr(results, "_render_persistence_footer", lambda *_a, **_k: None)
    monkeypatch.setattr(results.prompt_utils, "prompt_text", lambda *_a, **_k: (_ for _ in ()).throw(AssertionError("prompt_text should not be called for aborted runs")))
...
def test_render_results_aborted_uses_compact_summary(tmp_path: Path, monkeypatch, capsys) -> None:
    monkeypatch.chdir(tmp_path)
    now = datetime.now(UTC)
    manifest = SimpleNamespace(app_label="Tmp", package_name="tmp")
    report = SimpleNamespace(
    ...
    outcome.total_artifacts = 3
    outcome.completed_artifacts = 1
    params = RunParameters(
        profile="full",
        scope="app",
        scope_label="tmp",
        session_stamp="sess-aborted-compact",
        persistence_ready=True,
        dry_run=False,
        paper_grade_requested=False,
        verbose_output=False,
    ...
    assert "Static analysis interrupted" in out
    assert "Artifacts completed: 1/3" in out
    assert "Static analysis summary" not in out
    assert "Stage Context" not in out
    assert "Interpretation" not in out
    assert "findings (normalized) not persisted during dry run." not in out
