...
def test_emit_selection_manifest_writes_capture_distribution(tmp_path: Path, monkeypatch) -> None:
    ...
    assert payload["split_scan_enabled"] is True
    assert payload["artifact_count"] == 1
    assert payload["capture_distribution"] == {"20260217": 1}
    assert len(payload["artifact_manifest_sha256"]) == 64


def test_emit_selection_manifest_respects_base_only_split_policy(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    base_path = tmp_path / "repo" / "20260217" / "com.example.app" / "base.apk"
    split_path = tmp_path / "repo" / "20260217" / "com.example.app" / "split_config.en.apk"
    base_path.parent.mkdir(parents=True, exist_ok=True)
    base_path.write_text("apk", encoding="utf-8")
    split_path.write_text("apk-split", encoding="utf-8")
    ...
    split = RepositoryArtifact(
        path=split_path,
        display_path="20260217/com.example.app/split_config.en.apk",
        metadata={
            "package_name": "com.example.app",
            "session_stamp": "20260217",
            "split_group_id": 1,
    ...
    group = ArtifactGroup(
        ...
        session_stamp="20260217",
        capture_id="20260217",
        artifacts=(base, split),
    )
    selection = ScopeSelection(scope="all", label="All apps", groups=(group,))

    run_dispatch._emit_selection_manifest(  # noqa: SLF001 - contract guard
        selection,
        "20260216",
        scan_splits=False,
    )
    ...
    out = tmp_path / "output" / "audit" / "selection" / "20260216_selected_artifacts.json"
    payload = json.loads(out.read_text(encoding="utf-8"))
    assert payload["split_scan_enabled"] is False
    assert payload["artifact_count"] == 1
    assert payload["capture_distribution"] == {"20260217": 1}
    assert payload["apps"][0]["artifact_count"] == 1
    assert payload["apps"][0]["artifacts"] == ["20260217/com.example.app/base.apk"]
