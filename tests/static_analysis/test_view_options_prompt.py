...
def test_run_command_for_existing_session_audit_never_builds_or_launches(monkeypatch):
    """The audit choice for a reused label must stop before scan dispatch."""
    ...
    monkeypatch.setattr(actions, "_lookup_existing_session_state", lambda _stamp: (True, 1, 99))
    monkeypatch.setattr(actions.prompt_utils, "get_choice", lambda *_a, **_k: "2")
    audit_result = actions.prompt_run_setup(
        RunParameters(profile="full", scope="app", scope_label="Signal", session_stamp="used-label"),
        selection,
        command,
    )
    assert audit_result[0] == "cancel"

    monkeypatch.setattr(menu, "apply_command_overrides", lambda params, _command: params)
    monkeypatch.setattr(
        menu,
        "prompt_run_setup",
        lambda *_args: audit_result,
    )
