        ...
        json.dumps(
            {
                ...
                "summary": {
                    ...
                    "rows_with_network_signature": sum(1 for row in rows if row.get("network_signature")),
                },
            }
        ),
        encoding="utf-8",
...
def test_sdk_inventory_detector_reports_network_overlap(monkeypatch, tmp_path: Path) -> None:
    ...
    assert result.metrics["Top categories"] == [
        {"category": "Advertisement", "match_count": 1}
    ]
