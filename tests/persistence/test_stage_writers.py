...
def test_persist_metrics_and_sections_stage_rejects_false_contributor_write() -> None:
    noted: list[str] = []
    stage_context = SimpleNamespace(
        package_for_run="com.example.app",
        session_stamp="sess-1",
        scope_label="all",
        metrics_bundle=SimpleNamespace(contributors=[{"kind": "x"}]),
    ...
    try:
        persist_metrics_and_sections_stage(
            ...
            write_metrics=lambda *_a, **_k: True,
            write_contributors=lambda *_a, **_k: False,
            persist_static_sections_wrapper=lambda **_kwargs: ([], False, 0),
        )
    except RuntimeError as exc:
        assert "contributors.write:returned_false:run_id=11" in str(exc)
    else:
        raise AssertionError("expected contributors false-return to raise")
