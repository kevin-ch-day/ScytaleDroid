...
def test_persist_permission_risk_requires_static_run_id():
    session = "20250101-000099"
    report = DummyReport({"apk_id": 9999, "sha256": "aa" * 32})
    bundle = _bundle(1, 0, 0, 1.5, "B")

    with pytest.raises(RuntimeError, match="permission_risk.write requires static_run_id"):
        persist_permission_risk(
            run_id=99,
            static_run_id=None,
            report=report,
...
def test_persist_permission_risk_uses_static_run_id_for_vnext(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "upsert_vnext",
        lambda payload: captured.__setitem__("run_id", int(payload["run_id"])),
    )
