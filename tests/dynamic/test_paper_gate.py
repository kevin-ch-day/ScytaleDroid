...
def test_paper_gate_passes_minimal_contract(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_json(
        freeze_path,
        {
            ...
            "min_pcap_bytes_used": 50000,
            "qa_thresholds": {"min_pcap_bytes": 50000},
        },
    )
...
def test_paper_gate_fails_missing_linkage(tmp_path: Path, monkeypatch) -> None:
    ...
    run_dir = evidence_root / run_id
    ml_dir = run_dir / "analysis" / "ml" / "v1"
    _write_json(freeze_path, {"included_run_ids": [run_id], "min_pcap_bytes_used": 50000})
    _write_json(run_dir / "inputs" / "static_dynamic_plan.json", {"package_name": "com.example.app", "version_code": "123", "run_identity": {}})
    _write_json(run_dir / "run_manifest.json", {"target": {"package_name": "com.example.app", "version_code": "123"}})
...
def test_paper_gate_fails_on_freeze_min_pcap_mismatch(tmp_path: Path, monkeypatch) -> None:
    ...
    run_dir = evidence_root / run_id
    ml_dir = run_dir / "analysis" / "ml" / "v1"
    _write_json(freeze_path, {"included_run_ids": [run_id], "min_pcap_bytes_used": 100000})
    freeze_sha = __import__("hashlib").sha256(freeze_path.read_bytes()).hexdigest()
    _write_json(
        run_dir / "inputs" / "static_dynamic_plan.json",
        {
