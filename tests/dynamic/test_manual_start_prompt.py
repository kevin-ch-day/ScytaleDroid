...
def test_manual_start_prompt_allows_immediate_start_on_enter(monkeypatch, capsys) -> None:
    ...
    assert prompts == ["Press Enter to continue, or P to view raw high-value permissions"]
    assert "High-value permissions (sample)" not in capsys.readouterr().out


def test_manual_start_prompt_drains_stale_buffer_before_prompt(monkeypatch) -> None:
    run_ctx = SimpleNamespace(
        static_plan={"permissions": {"high_value": ["android.permission.CAMERA"]}}
    )
    drained = {"count": 0}
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: drained.__setitem__("count", drained["count"] + 1),
    )
    monkeypatch.setattr(manual.prompt_utils, "prompt_text", lambda *_args, **_kwargs: "")
...
def test_requires_explicit_begin_press_for_messaging_connected_baseline_only_when_not_immediate() -> None:
    ...
    assert manual._requires_explicit_begin_press(run_ctx=run_ctx, start_immediately=False) is True
    assert (
        manual._begin_capture_prompt_label(run_ctx)
        == "Press Enter to open live capture console and start connected-idle baseline..."
    )
...
def test_requires_explicit_begin_press_skips_second_prompt_for_non_messaging_immediate_start() -> None:
    ...
    assert manual._requires_explicit_begin_press(run_ctx=run_ctx, start_immediately=True) is False
    assert manual._begin_capture_prompt_label(run_ctx) == "Press Enter to open live capture console and start capture..."
