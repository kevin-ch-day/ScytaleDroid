...
def test_probe_finding_evidence_hash_parity_help_lists_script() -> None:
    repo = Path(__file__).resolve().parents[2]
    script = repo / "scripts" / "db" / "probe_finding_evidence_hash_parity.py"
    proc = subprocess.run(
        [sys.executable, str(script), "--help"],
        cwd=str(repo),
        capture_output=True,
    ...
    assert "parity" in out or "canonical" in out
