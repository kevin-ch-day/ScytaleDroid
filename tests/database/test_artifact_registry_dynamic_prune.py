...
def _row(
    *, artifact_id: int, run_id: str, host_path: str, primary_reason: str = "truly_detached"
) -> dict[str, object]:
    return {
...
def test_build_dynamic_prune_proposal(monkeypatch, tmp_path: Path) -> None:
    ...
    proposal = prune.build_dynamic_prune_proposal(
        fake_run_sql, repo_root=tmp_path, expected_count=2, expected_run_count=2
    )
...
def test_write_dynamic_prune_receipts(tmp_path: Path) -> None:
    proposal = prune.DynamicPruneProposal(
        ...
        target_rows=(
            {"artifact_id": 1, "resolved_dynamic_run_id": "11111111-1111-4111-8111-111111111111"},
        ),
        exact_sql_predicate="x",
        apply_delete_sql="y",
    ...
    prune.write_dynamic_prune_receipts(
        tmp_path,
        stem="artifact_registry_dynamic_prune_test",
        proposal=proposal,
    ...
    payload = json.loads(
        (tmp_path / "artifact_registry_dynamic_prune_test.json").read_text(encoding="utf-8")
    )
