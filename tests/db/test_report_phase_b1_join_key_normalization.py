...
def test_report_phase_b1_join_key_normalization_help_is_safe() -> None:
    repo = Path(__file__).resolve().parents[2]
    script = repo / "scripts" / "db" / "report_phase_b1_join_key_normalization.py"
    proc = subprocess.run(
...
def test_apply_phase_b1_join_key_normalization_help_is_safe() -> None:
    repo = Path(__file__).resolve().parents[2]
    script = repo / "scripts" / "db" / "apply_phase_b1_join_key_normalization.py"
    proc = subprocess.run(
        [sys.executable, str(script), "--help"],
        cwd=str(repo),
        capture_output=True,
